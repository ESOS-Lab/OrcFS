...
preOccupy()
{
	...
	WRITTEN=`echo $TotalSize*70/100 | bc`
...
#	sync
	...
	f2fs)
		...
		cat /proc/fs/f2fs/${DEVP}/valid_blocks_info > $VALIDBLOCKSNAME ;;
	ext4)
		cat /proc/fs/ext4/${DEVP}/mb_groups  > $STATNAME ;;
	esac
...
#	read -n 1 $input1
	...
	do
		...
		f2fs)
			...
			cat /proc/fs/f2fs/${DEVP}/valid_blocks_info > ${VALIDBLOCKSNAME%.txt}_${i}.txt ;
...
#                      mkfs.f2fs -s 128 -t 0 $DEV 83886080       # 40GByte
                      mkfs.f2fs -s 128 -t 0 $DEV 468862128       # 223GByte
		      echo " mkfs f2fs successful" ;; 
		ext4)
#		      mkfs -t ext4 -L ext4 -E nodiscard -E lazy_itable_init=0 -E lazy_journal_init=0 $DEV
                      mkfs -t ext4 -L ext4 -E nodiscard $DEV 2097152 # 2GByte
#                      mkfs -t ext4 -L ext4 -E nodiscard $DEV 20971520 # 20GByte
...
#		precondition
		...
		VALIDBLOCKSNAME=./result/${FileSystem}/03a_valid_blocks_preWritten.txt
		preOccupy
