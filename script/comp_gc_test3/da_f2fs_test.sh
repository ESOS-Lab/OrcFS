...
#	PreOccupySize_1K=`echo $TotalSize_1K*70/100 | bc`
	ToWriteSize_1K=`echo $TotalSize_1K*88/100 | bc`
	FileSize_1K=131072
...
#	./program  path  #_of_thread file_size_1k  record_size_1k  test_mode
	./comp_gc_test3	$MNT 100 $ToWriteSize_1K 32 0
#	./comp_gc_test3	$MNT 2 128 4 0
...
#	Get Filesystem Information
	...
	do
		...
		load_avg=`uptime`
		load_avg=`echo ${load_avg##*average} | awk '{print $2}'`
		load_avg=`echo "${load_avg%,}" | bc`
		load_avg=$(echo "$load_avg * 100" | bc)
		load_avg=`echo "${load_avg%.*}" | bc`

		while [ $load_avg -gt 200 ];
		do
			sleep 5
...
#		./program  path  #_of_thread file_size_1k  record_size_1k  test_mode
		./comp_gc_test3 $MNT 30 $ToWriteSize_1K 32 1
#		./comp_gc_test3 $MNT 2 128 4 1
