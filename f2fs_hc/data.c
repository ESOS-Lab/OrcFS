...
void list_hot_candidate_move_head(struct list_head *move)
{
	...
	mutex_lock(&hot_candidate_ilist_lock);
	list_move(move, &f2fs_hot_candidate_ilist);
//	list_del(move);
...
void inc_wcount(struct inode *inode)
{
	...
	if(!is_inode_hc_flag_set(fi, F2FS_HOT_FILE) 
		...
		/* Demotion the entry from hot list to the candidate list */		
...
#ifdef F2FS_BLOCK_LEVEL_HC_SEPARATION
void f2fs_hc_update_file_hotness(struct inode* inode)
{
	int ret;
	struct f2fs_inode_info *fi = F2FS_I(inode);
//	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	...
	if(fi == NULL){
		printk("[JS DBG] get f2fs_inode_info fail!\n");
		return;
	}
...
repeat:
	...
	if(fi->i_hc_addr != 0){
		...
		if(err == 0){
			printk("[JS DBG] read hc_file_info_fail!\n");
			return -1;
		}
...
void init_hc_file_info(struct f2fs_hc_file_info* hc_file_i)
{
	hc_file_i->nblocks = 0;
	hc_file_i->nwritten = 0;
	mutex_init(&hc_file_i->hc_file_mutex);
}

int read_hc_file_info_page(struct f2fs_sb_info *sbi, struct page *page, block_t blk_addr, int rw)
{
	return f2fs_submit_page_bio(sbi, page, blk_addr, rw);
}

block_t f2fs_hc_get_new_hc_addr(struct inode* inode, int hc_type)
{
	...
	curseg = CURSEG_I(sbi, hc_type);

	/* Lock curseg mutex */
	mutex_lock(&curseg->curseg_mutex);

	/* Get new_hc_addr block address*/
	new_hc_addr = NEXT_FREE_BLKADDR(sbi, curseg);
	...
	if (!__has_curseg_space(sbi, hc_type))
		sit_i->s_ops->allocate_segment(sbi, hc_type, false);

	/* Invalid sit entry for the dummy block address */
	update_sit_entry(sbi, new_hc_addr, 1);
	locate_dirty_segment(sbi, GET_SEGNO(sbi, new_hc_addr));
