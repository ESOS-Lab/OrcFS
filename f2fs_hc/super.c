...
static int block_copy_info_seq_show(struct seq_file *seq, void *offset)
{
	int i;
//TEMP
	seq_printf(seq, "n_blks\tsec_no\tn_free\tsectyp\tgc_type\tseclat\ttotlat\tcold_d\tdnodes\tdnodes2\n");
...
#ifdef F2FS_GET_FS_WAF
static int waf_info_open_fs(struct inode *inode, struct file *file)
{
	return single_open(file, waf_info_seq_show, PDE_DATA(inode));
}
#endif
...
#ifdef F2FS_GET_FS_WAF
static const struct file_operations f2fs_waf_info_fops = {
	.owner = THIS_MODULE,
	.open = waf_info_open_fs,
	.read = seq_read,
	.llseek = seq_lseek,
...
try_onemore:
	...
	for(i = 0; i < NR_HOTNESS_GROUP; i++){
		sbi->hc_group_info[i].nblocks = le64_to_cpu(sbi->ckpt->hc_group[i].nblocks);
		sbi->hc_group_info[i].total_hotness = le64_to_cpu(sbi->ckpt->hc_group[i].total_hotness);
		mutex_init(&sbi->hc_group_info[i].hc_group_mutex);
        }
#endif
	...
	build_gc_manager(sbi);
        ...
        block_copy_node_blocks2 = kmalloc(sizeof(unsigned int)*max_block_copy_index, GFP_KERNEL);
        ...
        if(!block_copy || !block_copy_free || !block_copy_secno || !block_copy_type || !gc_sec_latency || !gc_total_latency || !gc_type_info || !block_copy_cold_data_blocks || !block_copy_node_blocks || !block_copy_node_blocks2){
                err = -ENOMEM;
                goto free_nm;
        }
#endif
